...
table.insert(g_items, 	{
		...
		spawn_check = function() 
			local keyg = GlobalsGetValue("snowcastlekeygenerated", "0")
			if keyg == "0" then
				GlobalsSetValue("snowcastlekeygenerated", 1)
				if Portal_Key_Debug == 1 then GamePrint("One Key Generated (Hiisi Base, g_item)")	end
				return true
			else
